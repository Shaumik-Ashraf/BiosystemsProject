...
def reaction_helper(cpdB, 
	...
	flag = True; #pops this stack if false
	...
	if ( intermediate in all_past_intermediates ):
		if rdepth < all_intermediate_depths[ all_past_intermediates.index(intermediate) ]:
			...
			flag = False;
			if verbose:
				print("\tReject visited intermediate");
			return [False];
	elif(intermediate not in blacklisted_compounds):
	...
	for p in otherside:
		...
		for next_reaction in next_rxn_list:
			print("\trecurse at P:{0} | Next:{1}".format(p, next_reaction))
...
def GIBBS(RN,unknownreactions = 0): #Finds the Gibbs free energy for any kegg reaction via the reaction num
	try:
		GFE = 0;
